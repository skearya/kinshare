...
use iced::widget::{Stack, button, center, column, shader, text};
use iced::{Element, Length, Subscription, Theme, wgpu};

use server::{Screen, Server};
...
impl State {
    ...
    fn update(&mut self, message: Message) {
        match message {
            ...
            Message::Update(screen) => self.screen = Some(screen),
        }
    }
    ...
    iced::stream::channel(64, async |mut output| {
        ...
        loop {
            match receiver.recv().await {
                Ok(screen) => output.send(Message::Update(screen)).await.unwrap(),
                Err(err) => panic!("{err:#?}"),
            }
        }
    })
